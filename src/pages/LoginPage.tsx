...
import { AlertTriangle, ArrowRight, Eye, EyeOff, KeyRound, Loader2, Lock, Mail, MapPin } from 'lucide-react';
...
function friendlyAuthError(message?: string) {
  const text = message || 'Unable to sign in. Please try again.';
  const lower = text.toLowerCase();

  if (lower.includes('invalid login') || lower.includes('invalid credentials')) {
    return 'That email/password combination was not found. Check your password or create an account.';
  }
  if (lower.includes('email not confirmed')) {
    return 'Please confirm your email first, then come back and log in with your password.';
  }
  if (lower.includes('rate limit')) {
    return 'Too many email-link requests were sent. Use your password to log in, or wait before requesting another email link.';
  }
...
export default function LoginPage() {
  ...
  const [loading, setLoading] = useState<'password' | 'magic' | 'reset' | 'google' | null>(null);
  ...
  const handleMagicLink = async () => {
    if (!email.trim()) {
      setMessage({ type: 'error', text: 'Enter your email first, then request a backup login link.' });
      return;
    }
    ...
    if (error) {
      ...
      setMessage({ type: 'success', text: 'Backup login link sent. For normal logins, use your password instead.' });
    }
    setLoading(null);
  };
    ...
    <div className="min-h-screen bg-olive-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans selection:bg-brand-900/50">
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center px-4">
        ...
        <p className="mt-2 text-center text-sm text-olive-400">
          Or{' '}
          <Link to={signupHref} className="font-semibold text-brand-400 hover:text-brand-300 transition-colors">
        ...
        <div className="bg-olive-900/50 py-8 px-4 shadow-xl border border-surface-border sm:rounded-2xl sm:px-10 backdrop-blur-sm">
          {inAppBrowser && (
            <div className="mb-5 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-100 flex gap-2">
              ...
              <p>
                Facebook/Messenger browsers can block Google sign-in. If Google says load failed, tap the menu and open this page in Safari or Chrome.
              </p>
            </div>
          )}
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-olive-900 text-olive-500">Or log in with password</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-3 text-center text-sm">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 text-olive-300 hover:text-white disabled:opacity-60"
            >
              <KeyRound size={14} />
              Forgot password?
            </button>
            <button
              ...
              className="text-olive-500 hover:text-olive-200 disabled:opacity-60"
            >
              {loading === 'magic' ? 'Sending backup link...' : 'Email me a backup login link'}
            </button>
          </div>
        </div>
      </div>
