/**
 * Run conversion with updated input CSV containing Acquisition_Type for 300 off-market rows
 */
import fs from 'fs';
...
const INPUT = path.join(ROOT, 'data', 'input', 'updated_with_acquisition_types.csv');
...
async function main() {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    // Acquisition_Type: only auto-fill when confident (but skip if already filled).
